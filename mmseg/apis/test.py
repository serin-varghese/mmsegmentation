...
def warp_flow_val(img, flow):
    ...
    output = F.grid_sample(
        image, vgrid_scaled, mode='nearest', align_corners=True)
...
def class_based_iou(warped_img, pred2, label_id, min_size=0):
    ...
    pred1_binary = get_detected_pixels(
        warped_img, labels[label_id].color)  # act as gt image
    pred2_binary = get_detected_pixels(
        pred2, labels[label_id].color)  # act as actual image
...
def calculate_stability_city_saaid(flow, pred_1, pred_2):
    ...
    labels = labels_19_IDtoRGB
    for label in labels:
        iou_class, intersec, union = class_based_iou(warped_img, pred_2,
                                                     label.id, label.minsize)
        stabilities[label.name] = (iou_class.item(), intersec.item(),
                                   union.item())
        if iou_class.item() != -1:
    ...
    with torch.no_grad():
        for i in range(warped_img.shape[1] // step):
            for j in range(warped_img.shape[2] // step):
                ssim_val = ssim(
                    warped_img[:, step * i:step * (i + 1),
                               step * j:step * (j + 1)].unsqueeze(0),
                    curr_2[:, step * i:step * (i + 1),
                           step * j:step * (j + 1)].unsqueeze(0),
                    data_range=255,
                    size_average=False,
                    win_size=11,
...
def collect_tc(TC, stabilites_c):
    ...
    if rank == 0:
        ...
        for single_stabilities in stabilites_c:
            ...
            # previous = preds
        # print("stability values dict", stability_values)
            ...
            for img, img_meta in zip(imgs, img_metas):
                ...
                if out_dir:
                    if 'ppm' in img_meta['ori_filename']:
                        out_file = osp.join(
                            out_dir,
                            img_meta['ori_filename'].split('converted/')
                            [1].split('.ppm')[0] + '.png',
                        )
                        out_file_err = osp.join(
                            out_dir,
                            img_meta['ori_filename'].split('converted/')
                            [1].split('.ppm')[0] + '_error.png',
                        )
                    else:
                ...
                if out_dir:
                    if 'ppm' in img_meta['ori_filename']:
                        out_file = osp.join(
                            out_dir,
                            img_meta['ori_filename'].split('converted/')
                            [1].split('.ppm')[0] + '_t_' + str(t) + '.png',
                        )
                        out_file_f = osp.join(
                            out_dir,
                            img_meta['ori_filename'].split(
                                'converted/')[1].split('.ppm')[0] +
                            '_flow_t_' + str(t) + '.png',
                        )
                    else:
                        out_file = osp.join(
                            out_dir,
                            img_meta['ori_filename'].split('.png')[0] + '_t_' +
                            str(t) + '.png',
                        )
                        out_file_f = osp.join(
                            out_dir,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            if 'seq' in data:
                # print(data.keys())
                result, seq_pred = model(
                    **data, return_loss=False, rescale=True)
                # print(data.keys())
                # result = model(**data,return_loss=False, rescale=True)
                # for s_im in data["seq"]:
                # print("length of result", len(result))
                # print("length of seq result", len(seq_pred))
